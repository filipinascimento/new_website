...
function publicationsMarkdown(profile, works, fetchedAt) {
  const lines = [
    "---",
    "title: Publications",
    `description: ${escapeYaml("A generated scholarly record reconciled across Filipi Nascimento Silva's OpenAlex author profiles.")}`,
    ...
    "",
    `OpenAlex primary profile: ${profile.worksCount} indexed works, ${profile.citedByCount} citations, h-index ${profile.hIndex}.`,
    "",
  ];
...
const works = mergeWorks([...worksByAuthor.flat(), ...manuallyLinkedWorks]);
const profile = {
  id: primary.id,
